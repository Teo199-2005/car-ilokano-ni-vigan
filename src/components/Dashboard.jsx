            ...
            {loading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                  ...
                  <div className="card-glassmorphism rounded-lg p-6 animate-slideUp" style={{ animationDelay: '300ms' }}>
                  ...
                  <div className="card-glassmorphism rounded-lg p-6 animate-slideUp" style={{ animationDelay: '400ms' }}>
                    <div className="flex items-center justify-between mb-6">
                      <h4 className="text-lg font-semibold text-gray-800 flex items-center">
                        <Car size={20} className="mr-2 text-purple-600" />
                        Trending Car Types
                      </h4>
                      <div className="flex space-x-2">
                        <button onClick={() => setRevenueFilter('week')} className={`px-3 py-1 text-xs rounded ${revenueFilter === 'week' ? 'bg-purple-500 text-white' : 'bg-gray-200 text-gray-600'}`}>Week</button>
                        <button onClick={() => setRevenueFilter('month')} className={`px-3 py-1 text-xs rounded ${revenueFilter === 'month' ? 'bg-purple-500 text-white' : 'bg-gray-200 text-gray-600'}`}>Month</button>
                        <button onClick={() => setRevenueFilter('year')} className={`px-3 py-1 text-xs rounded ${revenueFilter === 'year' ? 'bg-purple-500 text-white' : 'bg-gray-200 text-gray-600'}`}>Year</button>
                      </div>
                    </div>
                                ...
                                <div className="w-24 bg-gray-200 rounded-full h-2">
                                  <div 
                                    ...
                                    style={{ 
                                      ...
                                      backgroundColor: item.count === 0 ? '#E5E7EB' : colors[index] 
                                    }}
                                  ></div>
                                </div>
                                <span className={`text-sm font-semibold w-8 ${item.count === 0 ? 'text-gray-400' : 'text-gray-600'}`}>{item.count}</span>
                                <span className={`text-xs w-10 ${item.count === 0 ? 'text-gray-400' : 'text-gray-500'}`}>{item.percentage}%</span>
                              </div>
                            </div>
                            {index < chartData.carTypeDistribution.length - 1 && (
                              <div className="mt-4 relative">
                                <div className="absolute inset-0 flex items-center">
                                  <div className="w-full border-t border-gray-200"></div>
                                </div>
                                <div className="absolute inset-0 flex items-center mt-0.5">
                                  <div className="w-full border-t border-gray-100"></div>
                                </div>
                                <div className="absolute left-0 top-0 w-8 h-full bg-gradient-to-r from-white to-transparent"></div>
                                <div className="absolute right-0 top-0 w-8 h-full bg-gradient-to-l from-white to-transparent"></div>
                              </div>
                            )}
                          </div>
                        );
